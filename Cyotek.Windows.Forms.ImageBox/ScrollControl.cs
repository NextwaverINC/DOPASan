...
namespace Cyotek.Windows.Forms
{
    ...
    public partial class ScrollControl : Control
    {
        ...
        protected bool HScroll
        {
            ...
            {
               return GetHScroll();
                //return (NativeMethods.GetWindowLong(this.Handle, NativeMethods.GWL_STYLE) & NativeMethods.WS_HSCROLL) == NativeMethods.WS_HSCROLL;
            }
            set
            {
        ...
        public delegate bool GetHScrollDelegate();
        public bool GetHScroll()
        {
            if (this.InvokeRequired)
            {
                GetHScrollDelegate d = new GetHScrollDelegate(GetHScroll);
        ...
        protected bool VScroll
        {
            ...
            {
                return GetVScroll();
                //return (NativeMethods.GetWindowLong(this.Handle, NativeMethods.GWL_STYLE) & NativeMethods.WS_VSCROLL) == NativeMethods.WS_VSCROLL;
            }
            set
            {
                uint longValue = NativeMethods.GetWindowLong(this.Handle, NativeMethods.GWL_STYLE);
        ...
        public delegate bool GetVScrollDelegate();
        public bool GetVScroll()
        {
            if (this.InvokeRequired)
            {
                GetVScrollDelegate d = new GetVScrollDelegate(GetHScroll);
                this.Invoke(d, new object[] { });
                return false;
            }
            else
        ...
        private int SetScrollInfo(ScrollOrientation scrollbar, NativeMethods.SCROLLINFO scrollInfo, bool refresh) // ReSharper restore UnusedMethodReturnValue.Local
        {
            if (this.InvokeRequired)
            {
                SetScrollInfoDelegate d = new SetScrollInfoDelegate(SetScrollInfo);
                this.Invoke(d, new object[] { scrollbar, scrollInfo, refresh });
                return 0;
            }
            else
            {
        ...
        delegate int SetScrollInfoDelegate(ScrollOrientation scrollbar, NativeMethods.SCROLLINFO scrollInfo, bool refresh);
